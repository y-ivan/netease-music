...
export default {
    ...
    mounted () {
        ...
        window.addEventListener("resize", () => {
            if (!this.slide || !this.slide.enabled) return false

            clearTimeout(this.resizeTimer)

            this.resizeTimer = setTimeout(() => {
                if (this.slide.inInTransition) this._onScrollEnd()
    ...
    methods: {
        ...
        _setSlideWidth (isResize) {
            ...
            if (this.loop && !isResize) width += 2 * slideWidth

            this.$refs.slideGroup.style.width = width + "px"
        },
        _initSlide () {
            this.slide = new BScorll(this.$refs.slide, {
                scrollX: true,
                scrollY: false,
                momentum: false,
                snap: {
